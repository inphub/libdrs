...
int ini_putbool(const TCHAR *Section, const TCHAR *Key, long Value, const TCHAR *Filename)
{
  if (Value==1) return ini_puts(Section, Key, "TRUE", Filename); else return ini_puts(Section, Key, "FALSE", Filename);
}
